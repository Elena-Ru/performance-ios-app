...
class LikeNewsThreadControl: UIControl{
    ...
    func setLikeNewsControl(news : Item, cellIndex: Int) {
        newsItem = news
        let text: String = String((news.likes?.count ?? 0))
       // let text: String = String((news.likeCount))
        likeCount.text = text
        setLikeButton(isLike: (news.likes?.userLikes)!)
        //setLikeButton(isLike: (news.userLikes))
        likeCount.textColor = news.likes?.userLikes == 1 ? .systemRed : .lightGray
